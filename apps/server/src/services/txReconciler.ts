...
export class TxReconciler {
  ...
  async run(): Promise<{ checked: number; confirmed: number; failed: number }> {
    ...
    const open = await repos.trades.byStatus(['PENDING', 'SUBMITTING', 'SUBMITTED'], 'LIVE');
    ...
    for (const t of open) {
      ...
      if (receipt) {
        const after = await this.execution.applyReceipt(t.id, receipt);
        if (after.status === 'CONFIRMED') out.confirmed++;
        else out.failed++;
        continue;
      }
      const round = await repos.rounds.getById(t.roundId);
      const wallet = t.walletId === null ? undefined : await repos.wallets.get(t.walletId);
      if (!round || !wallet) continue;
      head ??= await reader.getHead();
      const buffer = (await repos.markets.get(t.marketId))?.bufferSeconds ?? 30;
      if (round.lockTime !== null && head.blockTimestamp > round.lockTime + buffer) {
        ...
        if (ledger.amount > 0n) {
          await this.execution.confirmFromLedger(t.id, 'bet found in contract ledger; receipt unavailable');
          out.confirmed++;
        } else {
          await this.execution.fail(
            t.id,
            'DROPPED',
            'transaction was not mined before the round locked',
            true,
          );
          out.failed++;
        }
