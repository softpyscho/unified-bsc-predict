...
async function enable(h: ReturnType<typeof makeHarness>, flags: { paper: boolean; live: boolean }) {
  const s = h.app.repos.strategies.bySlug('follow-last-winner')!;
  h.app.repos.strategies.setFlags(s.id, {
    enabled: true,
...
describe('paper trading end-to-end', () => {
  it('runs the full pipeline and exposes the results through the API', async () => {
    ...
    const { repos } = h.app;
    const decisions = repos.decisions.list({ strategyId: strategy.id }, { limit: 100, offset: 0 }).rows;
    ...
    for (const t of trades) {
      const known = repos.rounds.get(market.id, t.epoch - 2)!;
    ...
    expect(repos.audit.list({ type: 'TRADE_SETTLED' }, 100).length).toBe(settled.length);
    await h.app.close();
  });
  ...
  it('refunds a paper bet when the round is cancelled', async () => {
    ...
    await tick(h);
    const trade = h.app.repos.trades.all({ mode: 'PAPER', epoch })[0]!;
    ...
    await tick(h);
    const after = h.app.repos.trades.get(trade.id)!;
  ...
  it('records why a strategy did not trade live when the live gate is closed', async () => {
    ...
    const live = h.app.repos.decisions.list({ mode: 'LIVE' }, { limit: 10, offset: 0 }).rows;
    ...
    expect(h.app.repos.trades.all({ mode: 'LIVE' })).toHaveLength(0);
    await h.app.close();
  });
  ...
  it('isolates a broken strategy configuration (recorded NO_TRADE, others unaffected)', async () => {
    ...
    const momentum = h.app.repos.strategies.bySlug('momentum')!;
    h.app.repos.strategies.setFlags(momentum.id, { enabled: true });
    h.app.repos.db.run('UPDATE strategies SET config = ? WHERE id = ?', [
      '{"params":{"window":-5}}',
      momentum.id,
    ]);
    h.app.bot.start('test');
    for (const p of [600, 601, 602, 603]) await playRound(h, p * 1e8);
    const broken = h.app.repos.decisions.list({ strategyId: momentum.id }, { limit: 10, offset: 0 }).rows;
    expect(broken.length).toBeGreaterThan(0);
    expect(broken.every((d) => d.decision === 'NO_TRADE' && d.reason.startsWith('CONFIG_INVALID'))).toBe(
      true,
    );
    expect(h.app.repos.trades.all({ strategyId: s.id }).length).toBeGreaterThan(0);
    await h.app.close();
  });
  ...
  it('emergency stop halts evaluation immediately and requires an explicit reset', async () => {
    ...
    h.app.bot.start('test');
    for (const p of [600, 601, 602]) await playRound(h, p * 1e8);
    const before = h.app.repos.decisions.list({}, { limit: 100, offset: 0 }).total;
    h.app.bot.emergencyStop('test');
    await playRound(h, 603e8);
    expect(h.app.repos.decisions.list({}, { limit: 100, offset: 0 }).total).toBe(before);
    expect(() => h.app.bot.start()).toThrow(/reset/);
    expect(() => h.app.bot.resetEmergency(false)).toThrow(/acknowledge/);
    expect(h.app.bot.resetEmergency(true).status).toBe('STOPPED');
    expect(h.app.repos.audit.list({ type: 'EMERGENCY_STOP' }, 5)[0]!.severity).toBe('CRITICAL');
    await h.app.close();
  });
});
...
describe('live trading end-to-end (simulated contract)', () => {
  async function liveHarness(opts: Parameters<typeof makeHarness>[0] = {}) {
    const h = makeHarness({ live: true, ...opts });
    h.chain.fund(h.app.config.walletAddress!, bnbToWei('1'));
    await h.app.recovery.run();
    await enable(h, { paper: false, live: true });
    h.app.bot.start('test');
    h.app.bot.armLive('ENABLE LIVE TRADING');
    return h;
  }
  ...
  it('bets, confirms, settles and claims with exact wallet accounting', async () => {
    ...
    const trades = h.app.repos.trades.all({ mode: 'LIVE' });
    ...
    const after = h.app.repos.trades.all({ mode: 'LIVE' });
    ...
    expect(h.app.repos.audit.list({ type: 'PAYOUT_CLAIMED' }, 5)).toHaveLength(1);
    await h.app.close();
  });
  ...
  it('rejects before submission when the wallet cannot afford the bet', async () => {
    ...
    const h = makeHarness({ live: true, env: { MAX_BANKROLL_FRACTION: '1' } });
    ...
    h.app.bot.armLive('ENABLE LIVE TRADING');
    for (const p of [600, 601, 602, 603]) await playRound(h, p * 1e8);
    const d = h.app.repos.decisions
      .list({ mode: 'LIVE', decision: 'NO_TRADE' }, { limit: 10, offset: 0 })
  ...
  it('trips the circuit breaker after repeated execution failures', async () => {
    ...
    h.writer!.fault = 'reject-insufficient';
    await playRound(h, 603e8);
    const failed = h.app.repos.trades.all({ mode: 'LIVE', status: 'FAILED' });
    expect(failed).toHaveLength(2);
    expect(failed[0]!.errorClass).toBe('INSUFFICIENT_FUNDS');
    const bot = h.app.bot.view();
    expect(bot.status).toBe('PAUSED');
    expect(bot.liveArmed).toBe(false);
    expect(h.app.repos.audit.list({ type: 'RISK_LIMIT_TRIGGERED' }, 5)).toHaveLength(1);
    await h.app.close();
  });
  ...
  it('resolves an ambiguous broadcast from the receipt without re-sending', async () => {
    ...
    await tick(h);
    const t = h.app.repos.trades.all({ mode: 'LIVE', epoch })[0]!;
    expect(t.status).toBe('SUBMITTING');
    const res = await h.app.txReconciler.run();
    expect(res.confirmed).toBe(1);
    expect(h.app.repos.trades.get(t.id)!.status).toBe('CONFIRMED');
  ...
  it('recovers after a crash: no blind re-submission, dropped bet marked failed, live disarmed', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bsp-db-'));
    const dbFile = path.join(dir, 'app.db');
    const h1 = await liveHarness({ env: { DATABASE_URL: `file:${dbFile}` } });
    ...
    await tick(h1);
    const pending = h1.app.repos.trades.all({ mode: 'LIVE', epoch })[0]!;
    ...
    expect(h2.app.bot.view().liveArmed).toBe(false); // BOT_AUTO_RESUME_LIVE defaults to false
    expect(h2.app.repos.trades.get(pending.id)!.status).toBe('SUBMITTING'); // still before lock: undecided
    h2.chain.execute(602e8);
    h2.chain.advance(60);
    await h2.app.txReconciler.run();
    const resolved = h2.app.repos.trades.get(pending.id)!;
  ...
  it('imports bets made outside the app and detects external claims', async () => {
    ...
    const t = h.app.repos.trades.all({ mode: 'LIVE', source: 'IMPORTED' })[0]!;
